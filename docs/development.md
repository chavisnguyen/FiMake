...
Tool parity is enforced by `mcp/tests/contract/mcp-tools.test.ts` and `NODE_ONLY_TOOLS` in `mcp/src/tools/registry.ts`: 28 tools forward to the plugin, 7 have Node-side logic (`get-selection`, `create-image`, `create-svg`, `set-image-fill`, `export-asset`, `export-file`, `list-clients`).
