...
`mcp` publishes as `@chavisnguyen/fimake` via `.github/workflows/publish-npm.yml` on GitHub releases (OIDC trusted publishing). `mcp/server.json` is the MCP registry entry (`io.github.chavisnguyen/fimake`).
